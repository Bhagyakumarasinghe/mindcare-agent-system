...
export default function HomeScreen({ navigation }) {
  ...
  // Navigation එක check කිරීමට හදපු function එක
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.grid}>
          <QuickAccessCard 
            icon="🌿" 
            title="Mood Tracking" 
            desc="Log your emotions" 
            color="#F0FDF4" 
            onPress={() => handleNavigation('MoodTracking')}
          />
          <QuickAccessCard 
            icon="🌬️" 
            title="Stress Relief" 
            desc="Breathing & relaxation" 
            color="#F0F9FF" 
            onPress={() => handleNavigation('Relax')}
          />
          <QuickAccessCard 
            icon="📊" 
            title="Analytics" 
          ...
          <QuickAccessCard 
            icon="❓" 
            title="Assessment" 
            desc="Take a quick test" 
            color="#FFFBEB" 
            onPress={() => Alert.alert("Coming Soon", "Assessment feature is under development.")}
          />
        </View>
...
const QuickAccessCard = ({ icon, title, desc, color, onPress }) => (
  <TouchableOpacity 
    style={[styles.qCard, { backgroundColor: color }]} 
    onPress={onPress}
    activeOpacity={0.7} // Click එක පේන්න මේක උදව් වෙනවා
  >
